...
def set_position_along_route(drone, graph, distance_along_route):
    """Move drone to the given cumulative distance along its route.

    Does NOT set drone.completed — the caller owns that decision.
    Previously this function set completed=True as a side effect, which caused
    silent completion when called during initialisation with large initial_progress.
    """
    if not drone.route or len(drone.route) < 2:
        return

    route_length = route_total_length(graph, drone.route)
    distance_along_route = max(distance_along_route, 0.0)
    ...
    if distance_along_route >= route_length:
        ...
        return  # caller checks for completion

    remaining = distance_along_route
